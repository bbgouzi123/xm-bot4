...
def test():
    ...
    if pop.Exists(1, 0.5):
        ...
        with open("popup_dump.txt", "w", encoding="utf-8") as f:
            f.write("------------------ DUMPING PROFILE INFO ------------------\n")
            for c, d in auto.WalkControl(pop):
                try:
                    name = c.Name
                except:
                    name = "error_reading_name"
                f.write(f"Depth {d}: type={c.ControlTypeName}, name='{name}', class={c.ClassName}\n")
            f.write("----------------------------------------------------------\n")
