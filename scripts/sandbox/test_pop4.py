...
def test():
    ...
    for child in auto.GetRootControl().GetChildren():
        if child.ClassName == 'Qt51514QWindowIcon' and child.Name == '微信':
            w = child
            break
    ...
    print(f"Found Wechat: {w.Name}, {w.NativeWindowHandle}")
    w.SetActive()
    time.sleep(1)
    ...
    if pop.Exists(1, 0.5):
        ...
        for c, d in auto.WalkControl(pop):
            try:
                ...
                name = ""
            print(f"Depth {d}: type={c.ControlTypeName}, name='{name}', class={c.ClassName}")
        print("----------------------------------------------------------")
